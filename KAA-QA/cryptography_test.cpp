...
TEST(cryptography, protect_data)
{
	...
	const auto encrypted_data = KAA::cryptography::protect_data(data.data(), data.size());
	EXPECT_NE(data, encrypted_data);
	const auto decrypted_data = KAA::cryptography::unprotect_data(encrypted_data.data(), encrypted_data.size());
	EXPECT_EQ(data, decrypted_data);
}
