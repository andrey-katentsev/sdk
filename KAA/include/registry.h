...
namespace KAA
{
	namespace system
	{
		...
		public:
			...
			std::unique_ptr<registry_key> open_key(root_key section, const std::string& subkey, const key_access& desired_access);
			std::unique_ptr<registry_key> create_key(root_key section, const std::string& subkey, key_options options, const key_access& desired_access, const void* desired_security = nullptr);

			virtual ~registry() = default;

		private:
			virtual std::unique_ptr<registry_key> iopen_key(root_key section, const std::string& subkey, const key_access& desired_access) = 0;
			virtual std::unique_ptr<registry_key> icreate_key(root_key section, const std::string& subkey, key_options options, const key_access& desired_access, const void* desired_security) = 0;
		};
	}
}
